...
3. Draft a reply using **exact wording** I specified—not AI improvisation
...
Before beginning, these are good things to keep in mind:
...
| Label-scoped queries | To avoid sending to the wrong recipients (not a job ad), use Gmail's Filters and apply a Label to only those emails with a job description.
...
The useful pattern here isn't "AI reads my email." It's:

> **Filters route → local LLM classifies → code templates reply → human approves send.**

## What I asked for (the initial prompt)

The reply template had strict rules:
...
> I'm a US Citizen, legal name is Guy Klages, birth month and day are 0902, I don't have any vacations planned, I just started round 1 of 3 interviews with two other companies. Please email me for any other questions or send me the RTR.

And one hard constraint: **use Mistral locally**—no cloud LLM processing email content.

---

## Architecture: split brain vs. rules engine
...
└── com.guyklages.gmail-job-agent.plist  # Optional macOS background job
```
...
(`gmail.send` is required even for drafts; `modify` is for labels.)
...
2. **Test users** → Add `tofu.eater@gmail.com`
3. Save, wait ~1 minute, rerun `setup_auth.py`

You do **not** need Google verification for a personal single-user agent. Testing mode + your email as a test user is enough.
...
### Troubleshooting: Mistral ignores YES/NO
...
### Troubleshooting: "I don't have a python directory"
...
```
Authenticated successfully as tofu.eater@gmail.com
...
Pay-range detection uses regex—not the LLM:
...
The full reply is assembled from fixed strings—no generation step.

---

## Step 5: Evolving requirements (drafts, labels, unread)

The first version **auto-sent** replies and scanned all unread mail. After testing, I changed three things:
...
### 2. Gmail filter first, agent second
...
The agent never adds the Job label—it trusts upstream filtering.

### 3. Don't mark messages as read
...
### Troubleshooting: `launchctl unload` → `Unload failed: 5`
...
Error 5 on unload usually means "nothing to unload"—not a broken agent.

---

## Advice for others building Gmail agents

### 1. Start with drafts, not send

Auto-reply agents can embarrass you. Draft mode + human review is the right first ship.

### 2. Use Gmail filters as the first gate
...
### 3. Keep a processed label

Without `job-agent/processed`, every poll recreates drafts for the same thread. Idempotency matters.

### 4. Don't send PII to cloud LLMs
...
### 5. Put personal details in config, not source

Legal name, birth month/day, citizenship lines belong in `config.yaml` (gitignored)—not hardcoded in repo.

### 6. Test classification with real emails
...
### 7. OAuth Desktop app, not Web

Web OAuth clients need redirect URI gymnastics. Desktop app + `run_local_server()` is simpler for CLI agents.

### 8. Regex pay-range detection will miss edge cases

"$140K DOE", "competitive salary", "€120k"—tune patterns from real emails. When in doubt, include the California pay-range ask (false positive is safer than false negative).

### 9. Resume path: one canonical file

Point config at a single PDF. Update that file when you revise your resume; the agent always attaches the latest.

### 10. Know what you're automating

This agent drafts **responses to inbound job outreach**. It doesn't apply to jobs, parse ATS portals, or negotiate. Scope creep is how Gmail agents become maintenance nightmares.
