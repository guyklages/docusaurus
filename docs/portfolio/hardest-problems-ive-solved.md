...
|-------------------|------------------|
| PROBLEM: <br/> Web application ran way too slow. <br/> <br/> Lead Engineer designed the database table optimized for humans: <br/> ![Before](../../static/img/HP_SqlServer_table_BEFORE.png) <br/> <br/> His code that queried the database: <br/> - Used `SELECT *` which requires two trips over the network (once to get the list of fields, and once to get the desired field) <br/> - Had to search for which row contains the desired information | MY SOLUTION: Optimize the database for computers. <br/> <br/> I re-designed the table: <br/> ![After](../../static/img/HP_SqlServer_table_AFTER.png) <br/> <br/> I re-wrote his code: - Used `SELECT FieldName` which requires only 1 trip over the network to get the desired field <br/> - Had to search only 1 row for the desired information |
