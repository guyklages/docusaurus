...
|-------------------|------------------|
| PROBLEM: <br/> Web application ran way too slow. <br/> <br/> Lead Engineer optimized the database table for _humans_: <br/> ![Before](../../static/img/HP_SqlServer_table_BEFORE.png) <br/> <br/> His code that queried the database: <br/> - For each field, used `SELECT *` which requires two trips over the network (one to get the field list + one to get the desired field) <br/> - Had to search for which row contains the desired data | MY SOLUTION: <br/> I re-designed the database table and code. <br/> <br/> I optimized the database table for _computers_: <br/> ![After](../../static/img/HP_SqlServer_table_AFTER.png) <br/> <br/> I re-wrote his code: <br/> - For each field, used `SELECT FieldName` which requires only 1 trip over the network to get the desired field <br/> - Had to search only 1 row for the desired information |
