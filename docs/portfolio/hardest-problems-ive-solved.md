...
| | RESULTS: <br/> - 12,000+ learners thrilled with their improved English. <br/> - Disney English bought our English club in 2006. |
...
|-------------------|------------------|
| PROBLEM: <br/> Web application ran way too slow. <br/> <br/> Lead Engineer designed the SQL database table as: <br/> ![Before](../../static/img/HP_SqlServer_table_BEFORE.png) <br/> <br/> His code that queried the database was: <br/> `sOnlyCertifiedV=cmd.sql("SELECT * FROM tblSettings WHERE Field='Only_Certified_V';")` <br/> `sMaxReturnPrice=cmd.sql("SELECT * FROM tblSettings WHERE Field='Max_Return_Price';")` <br/> `sProdSupported =cmd.sql("SELECT * FROM tblSettings WHERE Field='Prod_Supported';")` <br/> `sRegSupported  =cmd.sql("SELECT * FROM tblSettings WHERE Field='Reg_Supported';")` <br/> `sTermsUpdated  =cmd.sql("SELECT * FROM tblSettings WHERE Field='Terms_Updated';")` <br/> `iTimeoutSec    =cmd.sql("SELECT * FROM tblSettings WHERE Field=Timeout_Sec;")` | MY SOLUTION: Optimize the database. <br/> <br/> I re-designed the SQL table as: <br/> ![After](../../static/img/HP_SqlServer_table_AFTER.png) <br/> <br/> I re-wrote his code as: <br/> <br/> `sOnlyCertifiedV=cmd.sql("SELECT Only_Certified_V FROM tblSettings;")` <br/> `sMaxReturnPrice=cmd.sql("SELECT Max_Return_Price FROM tblSettings;")` <br/> `sProdSupported =cmd.sql("SELECT Prod_Supported   FROM tblSettings;")` <br/> `sRegSupported  =cmd.sql("SELECT Reg_Supported    FROM tblSettings;")` <br/> `sTermsUpdated  =cmd.sql("SELECT Terms_Updated    FROM tblSettings;")` <br/> `iTimeoutSec    =cmd.sql("SELECT Timeout_Sec      FROM tblSettings;")` |
| | RESULT: <br/> My code ran ~12 times faster. |
