---
title: iOS SDK for Apple Pay payments
authors: [guyklages]
tags: [fintech]
---

_(I wrote this for a job interview's assignment)_
...
**Note**: <br/> Use the `.xcworkspace` file to open your project in Xcode instead of the `.xcodeproj` file.

#### Using Carthage

To use [Carthage](https://github.com/Carthage/Carthage), simply add `github “stripe/stripe-ios”` to your `Cartfile` and follow the [Carthage Installation instructions](https://github.com/Carthage/Carthage#adding-frameworks-to-an-application).
...
<mark>**Important note before starting**
To use Apple Pay, you need to add the Apple Pay capability to your app in Xcode. This requires creating a merchant ID with Apple first, as explained in the “Getting Started with Apple Pay” documentation.</mark>
...
**Important**: <br/> Before doing the next step, make sure the controller has returned with a `PKPayment`.
...
**Note**: <br/> In the example above, the `createTokenWithCard` is called when a save button is tapped. It is important that the createToken is not called before user has finished entering their card details.
...
**Note**: <br/> The completion callback above is Apple Pay-specific. If you are not using Apple Pay, the code is still mostly the same, but you would want to implement custom error and success handling.
